...
This AST is strictly semantic. It describes *intent*, not implementation.
...
public record ValidationRule(
    Expression FilterExpression,     // The 'IF' part
    Expression RequirementExpression // The 'MUST' part
) : AstNode;
...
// Set Operations
public record InSetExpression(
    Expression Target, 
    List<LiteralExpression> SetValues
) : Expression;

public record BetweenExpression(
    Expression Target,
    LiteralExpression LowerBound,
    LiteralExpression UpperBound
) : Expression;

// Terminals
public record IdentifierExpression(
    string[] Parts // [Schema, Table, Column] or just [Column]
) : Expression;

public record LiteralExpression(
    object Value, 
    LiteralType Type // String, Number, Boolean
) : Expression;

// Enums
public enum BinaryOperatorType {
    And, Or, Equal, NotEqual, GreaterThan, LessThan, GreaterThanOrEqual, LessThanOrEqual, Add, Subtract, Multiply, Divide
}
```
